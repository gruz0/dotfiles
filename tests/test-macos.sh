#!/usr/bin/env bash
# Test script for macOS installation
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
DOTFILES_DIR="$(dirname "$SCRIPT_DIR")"

echo "Testing macOS installation scripts..."
echo ""
...
if [[ -x "${DOTFILES_DIR}/install-macos.sh" ]]; then
...
if [[ -f "${DOTFILES_DIR}/packages/macos-brew.txt" ]]; then
    echo "✓ packages/macos-brew.txt exists"
else
    echo "✗ packages/macos-brew.txt not found"
    ((FAILED++))
fi

if [[ -f "${DOTFILES_DIR}/packages/macos-casks.txt" ]]; then
    echo "✓ packages/macos-casks.txt exists"
else
    echo "✗ packages/macos-casks.txt not found"
    ((FAILED++))
fi
